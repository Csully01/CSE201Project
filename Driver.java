...
public class Driver extends JFrame{
		...
		if(f.exists()) {
			try {
				...
				if(pastInfo.size()>0) {
					...
					meal= Integer.parseInt(pastInfo.get(pastInfo.size()-5))+1;
				}
			}catch (Exception e) {
				e.getStackTrace();
			}
	...
	public void CalendarPage()
	{
		...
		// Objects to not reprint the same meal
		ArrayList<String> usedMeals = new ArrayList<String>();
		boolean used = false;
		...
		try
		{
			...
			while (scanner.hasNextLine())
			{
				...
				if(temp.indexOf("Day:")>=0) {
					int mealIndex = temp.indexOf("Meal");
					int curDay = Integer.parseInt(temp.substring(5,mealIndex-1));
					int curMeal = Integer.parseInt(temp.substring(mealIndex+6).replaceAll("\\s", ""));
					for(int i=0;i<usedMeals.size();i++) {
						String curArr = usedMeals.get(i);
						int arrDay = Integer.parseInt(curArr.substring(0,curArr.indexOf(",")));
						int arrMeal = Integer.parseInt(curArr.substring(curArr.indexOf(",")+1));
						if(arrDay==curDay&&arrMeal==curMeal) {
							// Skips past re-entry
							scanner.nextLine();
							scanner.nextLine();
							scanner.nextLine();
